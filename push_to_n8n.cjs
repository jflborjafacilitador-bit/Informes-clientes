...
async function run() {
  const w = JSON.parse(fs.readFileSync('wf_live.json', 'utf8'));
  ...
  if (putData.id) {
    ...
    // Activar via endpoint oficial
    console.log("Activando workflow...");
    const actData = await req(`/${wId}/activate`, "POST", {});
    console.log("Activación:", actData);
  } else {
