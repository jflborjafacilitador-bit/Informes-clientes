...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  const isEscrituracion = role === 'escrituracion';

  // Escrituración: solo Inventario, Calculadora y Configuración
  if (isEscrituracion) {
    const escrituracionLinks = [
    ...
    return (
      <aside className={`sidebar${isOpen ? ' sidebar--open' : ''}`}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '2px' }}>
                <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>Escrituración</span>
                <span style={{
                  ...
                  <FileSignature size={9} style={{ display: 'inline', marginRight: '2px' }} />
                  Escrituraci&oacute;n
                </span>
              </div>
            </div>
          </div>
          <button onClick={onClose} className="sidebar-close-btn" style={{ background: 'transparent', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', padding: '4px' }}>
            <X size={20} />
          </button>
        </div>
        <nav style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
          {escrituracionLinks.map((link) => {
        ...
        <div style={{ padding: '1.5rem', borderTop: '1px solid var(--border-glass)' }}>
          <button onClick={() => signOut()} style={{ display: 'flex', alignItems: 'center', gap: '10px', width: '100%', padding: '10px', background: 'transparent', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', borderRadius: '8px', transition: 'color 0.2s' }}
