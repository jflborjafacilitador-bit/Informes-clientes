...
.then(data => {
  const ex = data.data.map(e => {
    ...
    try {
      ...
      from = whData.data.key.remoteJid;
      text = whData.data.message.conversation || whData.data.message.extendedTextMessage?.text;
    } catch(err) {}
    
    return { id: e.id, status: e.status, instance, event, fromMe, from, text, lastNode, error: err };
  });
  fs.writeFileSync('execs3.txt', JSON.stringify(ex, null, 2), 'utf8');
})
.catch(console.error);
